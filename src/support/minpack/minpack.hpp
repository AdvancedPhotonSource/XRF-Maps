...
//
//    Input, _T X[N], the vector whose norm is desired.
//
//    Output, _T ENORM, the norm of X.
//
...
//
template<typename _T>
void chkder ( int m, int n, _T x[], _T fvec[], _T fjac[], int ldfjac, _T xp[], _T fvecp[], int mode, _T err[] )
{
  _T eps;
  _T epsf;
  _T epslog;
...
//
//    Input, _T R[LR], the upper triangular matrix R stored by rows.
//
...
template<typename _T>
void dogleg ( int n, _T r[], int lr, _T diag[], _T qtb[], _T delta, _T x[], _T wa1[], _T wa2[] )
{
  _T alpha;
  _T bnorm;
  _T epsmch;
...
//
//    Input, _T X[N], the evaluation point.
//
...
//         not referenced.
template<typename _T>
void fdjac1 ( void fcn (void* usr_data, int n, _T x[], _T f[], int *iflag ), void* usr_data, int n, _T x[], _T fvec[], _T fjac[], int ldfjac, int *iflag, int ml, int mu, _T epsfcn, _T wa1[], _T wa2[] )
{
  _T eps;
  _T epsmch;
  _T h;
...
//
  ...
  //
template<typename _T>
void fdjac2 ( void fcn ( int m, int n, _T x[], _T fvec[], int *iflag ), int m, int n, _T x[], _T fvec[], _T fjac[], int ldfjac, int *iflag, _T epsfcn, _T wa[] )
{
  _T eps;
  _T epsmch;
  _T h;
...
//
  ...
  //
template<typename _T>
void qform ( int m, int n, _T q[], int ldq, _T wa[] )
{
    int i;
    int j;
    int k;
    ...
    //
template<typename _T>
void qrfac ( int m, int n, _T a[], int lda, bool pivot, int ipvt[],  int lipvt, _T rdiag[], _T acnorm[], _T wa[] )
{
  _T ajnorm;
  _T epsmch;
  int i;
...
//
  ...
  //
template<typename _T>
void r1mpyq ( int m, int n, _T a[], int lda, _T v[], _T w[] )
{
  _T c;
  int i;
  int j;
...
//
  ...
  //
template<typename _T>
bool r1updt ( int m, int n, _T s[], int ls, _T u[], _T v[], _T w[] )
{
  _T cotan;
  _T cs;
  _T giant;
...
//
//    Input, _T A[M*N], the M by N matrix.
//
//    Input, string TITLE, a title.
//
template<typename _T>
void r8mat_print ( int m, int n, _T a[], char* title )
{
...
//  Parameters:
//
//    Output, _T R8_TINY, a "tiny" R8 value.
//
template<typename _T>
_T r8_tiny ( )
{
    return 0.4450147717014E-307;
}
...
//
//    Output, _T R8_UNIFORM_01, a new pseudorandom variate,
//    strictly between 0 and 1.
//
template<typename _T>
_T r8_uniform_01 ( int *seed )
{
...
//
template<typename _T>
void r8mat_print_some ( int m, int n, _T a[], int ilo, int jlo, int ihi,  int jhi, char* title )
{
# define INCX 5
...
//
template<typename _T>
int hybrd ( void fcn (void* usr_data, int n, _T x[], _T fvec[], int *iflag ), void* usr_data, int n, _T x[], _T fvec[], _T xtol, int maxfev, int ml, int mu, _T epsfcn, _T diag[], int mode, _T factor, int nprint,   int nfev, _T fjac[], int ldfjac, _T r[], int lr, _T qtf[],  _T wa1[], _T wa2[], _T wa3[], _T wa4[] )
{
  _T actred;
  _T delta;
  _T epsmch;
...
//
    iflag = 2;
    fdjac1 ( fcn, usr_data, n, x, fvec, fjac, ldfjac, &iflag, ml, mu, epsfcn, wa1, wa2 );
...
//
template<typename _T>
int hybrd1 ( void fcn (void* usr_data, int n, _T x[], _T fvec[], int *iflag ), void* usr_data, int n, _T x[], _T fvec[], _T tol, _T wa[], int lwa )
{
  _T epsfcn;
  _T factor;
  int index;
...
//
  ...
  info = hybrd ( fcn, usr_data, n, x, fvec, xtol, maxfev, ml, mu, epsfcn, wa, mode,
