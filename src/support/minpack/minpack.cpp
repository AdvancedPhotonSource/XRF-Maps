...
//
//    Input, real_t R[LR], the upper triangular matrix R stored by rows.
//
...
//
//    Input, real_t X[N], the vector whose norm is desired.
//
//    Output, real_t ENORM, the norm of X.
//
{
  int i;
  real_t value;
...
//
//    Input, real_t X[N], the evaluation point.
//
...
int hybrd ( void fcn (void *usr_data, int n, real_t x[], real_t fvec[], int *iflag ),
  void *usr_data, int n, real_t x[],
  real_t fvec[], real_t xtol, int maxfev, int ml, int mu, real_t epsfcn,
  real_t diag[], int mode, real_t factor, int nprint, int nfev,
  real_t fjac[], int ldfjac, real_t r[], int lr, real_t qtf[], real_t wa1[],
...
int hybrd1 ( void fcn (void *usr_data, int n, real_t x[], real_t fvec[], int *iflag ), void *usr_data,
             int n, real_t x[], real_t fvec[], real_t tol, real_t wa[], int lwa )
...
bool r1updt ( int m, int n, real_t s[], int ls, real_t u[], real_t v[],
  real_t w[] )
...
//  Parameters:
//
//    Output, real_t R8_EPSILON, the R8 round-off unit.
//
{
  const real_t value = 2.220446049250313E-016;
...
//  Parameters:
//
//    Output, real_t R8_HUGE, a "huge" R8 value.
//
{
  real_t value;
...
//  Parameters:
//
//    Input, real_t X, Y, the quantities to compare.
//
//    Output, real_t R8_MAX, the maximum of X and Y.
//
{
  real_t value;
...
//    Input, real_t X, Y, the quantities to compare.
//
//    Output, real_t R8_MIN, the minimum of X and Y.
//
{
  real_t value;
...
//  Parameters:
//
//    Output, real_t R8_TINY, a "tiny" R8 value.
//
{
  real_t value;
...
//
//    Output, real_t R8_UNIFORM_01, a new pseudorandom variate,
...
//
//    Input, real_t A[M*N], the M by N matrix.
//
...
//
//    Input, real_t A[N], the vector to be printed.
//
//    Input, string TITLE, a title.
//
{
