...
struct mp_par
{
  ...
  _T step;      /* Step size for finite difference */
  _T relstep;   /* Relative step size for finite difference */
  ...
  int deriv_debug;  /* Derivative debug mode: 1 = Yes; 0 = No;
		       ...
		       (0, 1, -1, or 2).
		    */
  _T deriv_reltol; /* Relative tolerance for derivative debug
			  printout */
  _T deriv_abstol; /* Absolute tolerance for derivative debug
			  printout */
};
...
struct mp_config
{
  /* NOTE: the user may set the value explicitly; OR, if the passed
     ...
     mpfit(). */
  _T ftol;    /* Relative chi-square convergence criterium Default: 1e-10 */
  _T xtol;    /* Relative parameter convergence criterium  Default: 1e-10 */
  _T gtol;    /* Orthogonality convergence criterium       Default: 1e-10 */
  _T epsfcn;  /* Finite derivative step size               Default: MP_MACHEP0 */
  _T stepfactor; /* Initial step bound                     Default: 100.0 */
  _T covtol;  /* Range tolerance for covariance calculation Default: 1e-14 */
...
struct mp_result
{
  ...
  _T *resid;       /* Final residuals
			  nfunc-vector, or 0 if not desired */
  _T *xerror;      /* Final parameter uncertainties (1-sigma)
			  npar-vector, or 0 if not desired */
  _T *covar;       /* Final parameter covariance matrix
...
typedef int (*mp_func)(int m, // Number of functions (elts of fvec)
                       ...
                       _T *x,      // I - Parameters
                       _T *fvec,   // O - function values
                       _T **dvec,  // O - function derivatives (optional)
...
_T mp_enorm(int n, _T *x)
{
   ...
   *
   *	_T precision function enorm(n,x)
   *
   *     where
   *
...
template<typename _T>
int mp_fdjac2(int (*mp_func)(int m, int n, _T *x, _T *fvec, _T **dvec, void *private_data),
              int m, int n, int *ifree, int npar, _T *x, _T *fvec,
              _T *fjac, int ldfjac, _T epsfcn,
              _T *wa, void *priv, int *nfev,
              _T *step, _T *dstep, int *dside,
              int *qulimited, _T *ulimit,
              int *ddebug, _T *ddrtol, _T *ddatol,
              _T *wa2, _T **dvec)
{
/*
*     **********
...
*/
  ...
  if (has_analytical_deriv) {
    iflag = mp_call(mp_func, m, npar, x, wa, dvec, priv);
    if (nfev) *nfev = *nfev + 1;
  ...
  if (has_numerical_deriv) for (j=0; j<n; j++) {  /* Loop thru free parms */
    ...
    _T dr = ddrtol[ifree[j]], da = ddatol[ifree[j]];
    ...
    x[ifree[j]] = temp + h;
    iflag = mp_call(mp_func, m, npar, x, wa, 0, priv);
    if (nfev) *nfev = *nfev + 1;
    if (iflag < 0 ) goto DONE;
    x[ifree[j]] = temp;
...
template<typename _T>
void mp_qrfac(int m, int n, _T *a, int lda,
              int pivot, int *ipvt, int lipvt,
              _T *rdiag, _T *acnorm, _T *wa)
{
/*
*     **********
*
            ...
            if ((pivot != 0) && (rdiag[k] != zero))
              {
                ...
                temp = std::max( zero, one-temp*temp );
                rdiag[k] *= sqrt(temp);
                temp = rdiag[k]/wa[k];
...
template<typename _T>
void mp_qrsolv(int n, _T *r, int ldr, int *ipvt, _T *diag,
               _T *qtb, _T *x, _T *sdiag, _T *wa)
{
/*
*     **********
*
...
template<typename _T>
void mp_lmpar(int n, _T *r, int ldr, int *ipvt, int *ifree, _T *diag,
              _T *qtb, _T delta, _T *par, _T *x,
              _T *sdiag, _T *wa1, _T *wa2)
{
  /*     **********
   *
   *     subroutine lmpar
  ...
  paru = gnorm/delta;
  if (paru == zero)
    paru = MP_DWARF/std::min(delta,p1);
  /*
...
template<typename _T>
int mp_covar(int n, _T *r, int ldr, int *ipvt, _T tol, _T *wa)
{
  int i, ii, j, jj, k, l;
  int kk, kj, ji, j0, k0, jj0;
  int sing;
  _T one = 1.0, temp, tolr, zero = 0.0;
...
*     _T *xall   - array of n initial parameter values
...
template <typename _T>
int mpfit(int (*mp_func)(int m, int n, _T *x, _T *fvec, _T **dvec, void *private_data),
          int m, int npar, _T *xall, mp_par<_T> *pars, mp_config<_T> *config,
          void *private_data, mp_result<_T> *result)
{
  mp_config<_T> conf;
  ...
  int *ddebug = 0;
  _T *ddrtol = 0, *ddatol = 0;

  _T *fvec = 0, *qtf = 0;
  _T *x = 0, *xnew = 0, *fjac = 0, *diag = 0;
  _T *wa1 = 0, *wa2 = 0, *wa3 = 0, *wa4 = 0;
  _T **dvecptr = 0;
  int *ipvt = 0;
  ...
  mp_malloc(step,  _T, npar);
  mp_malloc(dstep, _T, npar);
  mp_malloc(mpside, int, npar);
  mp_malloc(ddebug, int, npar);
  mp_malloc(ddrtol, _T, npar);
  mp_malloc(ddatol, _T, npar);
  if (pars) for (i=0; i<npar; i++) {
    step[i] = pars[i].step;
  ...
  if (pars) {
    ...
    mp_malloc(ulim, _T, nfree);
    mp_malloc(llim, _T, nfree);
  ...
  mp_malloc(fvec, _T, m);
  mp_malloc(qtf, _T, nfree);
  mp_malloc(x, _T, nfree);
  mp_malloc(xnew, _T, npar);
  mp_malloc(fjac, _T, m*nfree);
  ldfjac = m;
  mp_malloc(diag, _T, npar);
  mp_malloc(wa1, _T, npar);
  mp_malloc(wa2, _T, npar);
  mp_malloc(wa3, _T, npar);
  mp_malloc(wa4, _T, m);
  mp_malloc(ipvt, int, npar);
  mp_malloc(dvecptr, _T *, npar);

  /* Evaluate user function with initial parameter values */
  iflag = mp_call(mp_func, m, npar, xall, fvec, 0, private_data);
  nfev += 1;
  if (iflag < 0) {
    goto CLEANUP;
  }
 ...
 OUTER_LOOP:
  ...
  iflag = mp_fdjac2(mp_func, m, nfree, ifree, npar, xnew, fvec, fjac, ldfjac,
  ...
  if (conf.nofinitecheck) {
    ...
    for (j=0; j<nfree; j++) {
      for (i=0; i<nfree; i++) {
        if (std::isfinite(fjac[off+i]) == 0) nonfinite = 1;
      }
      off += ldfjac;
    }
  ...
  if (fnorm != zero) {
    ...
    for (j=0; j<nfree; j++ ) {
      ...
      if (wa2[l] != zero) {
        ...
        gnorm = std::max(gnorm,std::abs(sum/wa2[l]));
      }
      jj += m;
    }
  }
  ...
  if (conf.douserscale == 0) {
    for (j=0; j<nfree; j++ ) {
      diag[ifree[j]] = std::max(diag[ifree[j]],wa2[j]);
    }
  }
 ...
 L200:
  ...
  if (qanylim == 0) {
    ...
    for (j=0; j<nfree; j++) {
      ...
      if (dwa1 && qllim[j] && ((x[j] + wa1[j]) < llim[j])) {
        alpha = std::min(alpha, (llim[j]-x[j])/wa1[j]);
      }
      if (dwa1 && qulim[j] && ((x[j] + wa1[j]) > ulim[j])) {
        alpha = std::min(alpha, (ulim[j]-x[j])/wa1[j]);
      }
    }
  ...
  iflag = mp_call(mp_func, m, npar, xnew, wa4, 0, private_data);
  nfev += 1;
 ...
 L300:
  ...
  if (result) {
    ...
    result->bestnorm = std::max(fnorm,fnorm1);
